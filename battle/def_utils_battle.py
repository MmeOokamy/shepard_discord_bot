...
def special_txt(player):
    return (
        f"Force : {player.strength} :muscle:, Perception : {player.perception} :eye:, "
...
def embed_atk(step, atk_a, atk_b, rip_a, rip_b, result):
    ...
    embed = discord.Embed(
        title=f"Round {step}", description=f"=)", color=discord.Colour.random()
    )
    # celui qui attak
...
def embed_adv(ctx, adv_id):
    ...
    file_embed = {"file": file, "embed": embed}
    return file_embed
