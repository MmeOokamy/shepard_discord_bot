...
def db_user_exist(user_id):
    return (
        False
        if c.execute(f"SELECT 1 FROM user WHERE user_id = '{user_id}'").fetchone()
        is None
        else True
    )
...
def db_fight_get_special_total(user_id, adv_id=0):
    ...
    s = (
        0
        if (int(special["strength"]) + int(player["strength"])) < 0
        else (int(special["strength"]) + int(player["strength"]))
    )

    p = (
        0
        if (int(special["perception"]) + int(player["perception"])) < 0
        else (int(special["perception"]) + int(player["perception"]))
    )

    e = (
        0
        if (int(special["endurance"]) + int(player["endurance"])) < 0
        else (int(special["endurance"]) + int(player["endurance"]))
    )

    ch = (
        0
        if (int(special["charisma"]) + int(player["charisma"])) < 0
        else (int(special["charisma"]) + int(player["charisma"]))
    )

    i = (
        0
        if (int(special["intelligence"]) + int(player["intelligence"])) < 0
        else (int(special["intelligence"]) + int(player["intelligence"]))
    )

    a = (
        0
        if (int(special["agility"]) + int(player["agility"])) < 0
        else (int(special["agility"]) + int(player["agility"]))
    )

    lu = (
        0
        if (int(special["luck"]) + int(player["luck"])) < 0
...
def db_fight_get_user_special_for_create_fighter(user_id):
    ...
    s = (
        0
        if (int(special["strength"]) + int(user["strength"])) < 0
        else (int(special["strength"]) + int(user["strength"]))
    )

    p = (
        0
        if (int(special["perception"]) + int(user["perception"])) < 0
        else (int(special["perception"]) + int(user["perception"]))
    )

    e = (
        0
        if (int(special["endurance"]) + int(user["endurance"])) < 0
        else (int(special["endurance"]) + int(user["endurance"]))
    )

    ch = (
        0
        if (int(special["charisma"]) + int(user["charisma"])) < 0
        else (int(special["charisma"]) + int(user["charisma"]))
    )

    i = (
        0
        if (int(special["intelligence"]) + int(user["intelligence"])) < 0
        else (int(special["intelligence"]) + int(user["intelligence"]))
    )

    a = (
        0
        if (int(special["agility"]) + int(user["agility"])) < 0
        else (int(special["agility"]) + int(user["agility"]))
    )

    lu = (
        0
        if (int(special["luck"]) + int(user["luck"])) < 0
...
def db_fight_get_adversary_by_id_for_create(adv_id, user_id):
    ...
    s = (
        0
        if (int(special["strength"]) + int(adv_select["strength"])) < 0
        else (int(special["strength"]) + int(adv_select["strength"]))
    )

    p = (
        0
        if (int(special["perception"]) + int(adv_select["perception"])) < 0
        else (int(special["perception"]) + int(adv_select["perception"]))
    )

    e = (
        0
        if (int(special["endurance"]) + int(adv_select["endurance"])) < 0
        else (int(special["endurance"]) + int(adv_select["endurance"]))
    )

    ch = (
        0
        if (int(special["charisma"]) + int(adv_select["charisma"])) < 0
        else (int(special["charisma"]) + int(adv_select["charisma"]))
    )

    i = (
        0
        if (int(special["intelligence"]) + int(adv_select["intelligence"])) < 0
        else (int(special["intelligence"]) + int(adv_select["intelligence"]))
    )

    a = (
        0
        if (int(special["agility"]) + int(adv_select["agility"])) < 0
        else (int(special["agility"]) + int(adv_select["agility"]))
    )

    lu = (
        0
        if (int(special["luck"]) + int(adv_select["luck"])) < 0
