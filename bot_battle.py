...
class BotBattle(commands.Cog):
    ...
    async def battle_game(self, ctx):
        ...
        async def battle(player, two):
            ...
            await ctx.send(
                embed=embed_atk(
                    step, atk_one_a, atk_one_b, atk_two_a, atk_two_b, atk_result
                )
            )
        ...
        await ctx.send(
            "Un petit combat ??", view=view_fs, delete_after=20, mention_author=True
        )
        await view_fs.wait()
        ...
        if view_fs.value is True:
            ...
            while player_one.alive and player_two.alive:
                ...
                await ctx.reply(
                    "Quelle action fais-tu ?", view=view_fc, mention_author=True
                )
                await view_fc.wait()
